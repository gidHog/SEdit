...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		private float reverseMult = 1.0f;
		...
		void HandleKeyInput()
		{
			...
			{
				StartCoroutine(HandleKeyInput(translatingType));

			}
			else if (Input.GetKeyUp(KeyCode.DownArrow))
			{
				/*
				 * Switches the Direction (amount *-1) , a bit hacky 
				 */
		...
		IEnumerator HandleKeyInput(TransformType transType)
		{
			...
			if (Input.GetKey(KeyCode.UpArrow))
			{
				movement.y += (speed * reverseMult);
				currentAxis.y = (1.0f * reverseMult);

			}
			else if (Input.GetKey(KeyCode.LeftArrow))
			{
				movement.x += (speed * reverseMult);
				currentAxis.x = (1.0f * reverseMult);
			}
			else if (Input.GetKey(KeyCode.RightArrow))
			{
				movement.z += (speed * reverseMult);
				currentAxis.z = (1.0f * reverseMult);
			}
			...
			else if (transType == TransformType.Scale)
			{
				...
				Vector3 targetScaleAmount = currentAxis * scaleAmount;
			...
			else if (transType == TransformType.Rotate)
			{
				...
				for (int i = 0; i < targetRootsOrdered.Count; i++)
				{
					...
					if (pivot == TransformPivot.Pivot)
					{
						target.Rotate(currentAxis, speed*10, Space.World);
					}
					else if (pivot == TransformPivot.Center)
					{
						target.RotateAround(originalPivot, currentAxis, speed * 10);
					}
				}

				totalRotationAmount *= Quaternion.Euler(axis * speed * 10);
			}
		...
		void GetTarget()
		{
		//currently disabled (i cant get the shader to work(for mouse input))
		/*	if(nearAxis == Axis.None && Input.GetMouseButtonDown(0))
			{
